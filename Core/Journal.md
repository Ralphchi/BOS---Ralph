...
## 2026-04-23 (début de soirée) — Deux objections fondamentales + reframe "compliance is the product"
- **Ralph a soulevé deux questions critiques avant le premier outreach (lundi 28/04) :**
  1. *« En quoi CRI est différent de l'advisor qui balance ses données dans ChatGPT et lui demande de rédiger ? Je ne vois pas ce qui est spécifique et notable. »*
  2. *« Le marché financier est très régulé et closed. Je ne sais pas si c'est super légal que la compagnie mette les données du client dans un software d'IA. Je ne pense pas que le client aurait envie que ça se passe, peur de data leaks, et ça pourrait mener à la perte du client pour la compagnie. »*
- **Diagnostic BOS :** les deux objections sont celles qu'un EAM sérieux posera en discovery call. Les réponses existent et sont solides, mais n'étaient pas codifiées dans les artefacts vendables (dossier, outreach pack, Business.md).
- **Reframe stratégique MAJEUR :** **compliance is the product, not a constraint.** CRI n'est pas « énième AI tool pour quarterly reviews ». CRI est *« the compliant, audited path to the productivity gain that 30-50% of Swiss advisors are already grabbing silently with ChatGPT »*. Ce framing déplace la conversation commerciale de « on vend de l'IA dans un marché sans IA » vers « on vend la version contrôlée dans un marché qui improvise dans l'ombre ».
  - **Implication produit :** le vrai moat est la combinaison 4 couches — voice-calibration expertise + compliance architecture + measurement rigor + ongoing service. Pas le prompt (copiable). Les 4 ensemble, délivrées par un humain (Ralph) qui connaît FINMA 08/2024 dans le détail et maintient l'infrastructure dans le temps.
  - **Implication commerciale :** la competition réelle n'est pas « doing nothing » (l'old guard qui écrit à la main) mais « naked ChatGPT silently » (l'advisor qui pastes cleartext). CRI = la version vettée de ce qui se fait déjà mal.
- **Artefacts mis à jour (5 deliverables exécutés ce soir) :**
  - **Dossier v1.3** : ajout Part 1 « What I'm competing with » (naked ChatGPT = vraie concurrence), ajout Part 3 tableau comparatif « Why not just ChatGPT? » (7 dimensions de différenciation + calcul ROI DIY CHF 12-27k vs CHF 13.9k CRI), ajout ouverture Part 5 « Compliance is the product, not a constraint »
  - **Outreach pack** : nouvelle section « Discovery call — handling the two critical objections » avec 2 réponses verbatim prêtes à voix haute + follow-ups + frame unifié + rehearsal rule (lire à voix haute 3× avant premier discovery call)
  - **Core/Business.md** : positioning reframe en tête (compliance-as-product) + explicitation du moat 4 couches
  - **Core/Journal.md** : entrée présente
  - **PDF re-rendered v1.3**
- **Principe préservé dans le cold outreach** : les 2 objections ne sont invoquées qu'en discovery call, pas en cold message. Le message LinkedIn/email reste sur le quarterly review + pilot CHF 2 400 + refund. Le framing compliance-as-product et le tableau vs ChatGPT vivent dans le dossier et le discovery call playbook, déclenchés quand l'advisor pose LES questions.
- **Course correction Ralph #7 d'affilée.** Pattern confirmé : son instinct skeptique sur les faiblesses produit/marché converge SYSTÉMATIQUEMENT vers un reframe qui renforce le positioning. Chaque pushback = upgrade structurel. À ce stade, **ne plus défendre la reco initiale face à un pushback de Ralph** — creuser directement et restructurer.
- **Enseignement stratégique (le plus important de cette session, peut-être du projet) :** CRI n'est pas un produit IA. CRI est **une infrastructure compliance pour un marché régulé qui improvise dans l'ombre**. Ce repositioning déplace le deal d'un achat opportuniste (gadget productivité) à un achat de sécurité (gestion du risque régulatoire). Un EAM achète à un niveau fondamentalement différent. C'est aussi pourquoi le pricing CHF 7 900 + 499/mois tient : on vend de la sécurité régulatoire, pas du logiciel.

## 2026-04-23 (nuit) — Course correction #8 : cadrage compétition 3-horizons + décision d'audit exhaustif
- **Pushback Ralph :** *« es-tu sur que tu as touché le bon angle sur la compétition ? »*
- **Diagnostic BOS :** non. La v1.3 claimait « naked ChatGPT used by 30-50% of advisors = real competition ». **Chiffre inventé, framing partiel.** Un compliance officer sérieux met ce claim en déroute en 10 secondes.
- **Vraie compétition structurée en 3 horizons (v1.4) :**
  - **Horizon 1 (aujourd'hui) :** hand-written ~55-65% (vrai statu quo dominant), naked ChatGPT silencieux ~15-25%, junior analyst ~10-15%, offshore ~5-10%
  - **Horizon 2 (12-24 mois) :** PMS vendors ajoutant AI commentary natively (Assetmax, WealthArc, WIZE, Expersoft) = vraie menace structurelle. Moat CRI = PMS-agnostic + pseudonymization enforced + compliance contractuelle directe
  - **Horizon 3 (24+ mois) :** consolidation. Exit ou leader compliance-first
- **Artefacts v1.4 mis à jour :** `strategy-dossier-v1.md` Part 1 réécrite, `Core/Business.md` section concurrentielle refactored 3-horizons, `outreach-pack-en.md` unifying frame reformulé sans chiffre inventé (« your advisors today write in one of four ways... in 12-24 months a fifth option arrives... CRI is the sixth — the only one that combines productivity and compliance structurally »).
- **Course correction Ralph #8 d'affilée.** Pattern désormais validé comme loi BOS : un pushback de Ralph sur un claim que je crois solide = j'ai très probablement raté quelque chose. Ne plus défendre, creuser direct.
- **Décision stratégique déclenchée par ce pushback : AUDIT EXHAUSTIF du projet entier.** Ralph a demandé : *« je ne veux plus que tu me reviennes chaque message avec "as-tu le bon angle ? non..." — absorbe tout une fois et produis des sections profondes. Je veux un oracle d'informations. »*
